...
\end{flushleft}
\begin{figure}[ht!]
	\centering\includegraphics[width=160mm]{../schema/gif/helix2-schema_2.png}
...
\begin{flushleft}
	...
	\textls[-50]{\mono{
		...
		A source in Helix2 architecture is called a {\mbold{cation}}.\linebreak\linebreak
...
\end{flushleft}
\begin{figure}[ht!]
	\centering\includegraphics[width=160mm]{../schema/ens.png}
	\mono{\caption{Ethereum Name Service (ENS)}}
	\label{fig:ens}
\end{figure}
\begin{figure}[ht!]
	\centering\includegraphics[width=160mm]{../schema/helix2.png}
...
\begin{flushleft}
	\textls[-50]{\mono{
		...
		All Helix2 names end with\tcbox{.} and they have a Resolver and Controller. Note again that\tcbox{.} is a reserved character and therefore forbidden. Additionally, \tcbox{color{purple}{-}} and\tcbox{\textcolor{purple}{\textcolor{purple}{\#}}} are also forbidden.
		}}\linebreak\linebreak\linebreak
	\textbf{\large\pbold{IMPORTING ENS NAMES}}\linebreak\linebreak
	\textls[-50]{\mono{
...
\begin{flushleft}
	\textls[-50]{\mono{
		A directional bond between two names \tcbox{alice.}$\rightarrow$\tcbox{bob.} is labeled by its \tcbox{label}. Bonds start with \tcbox{\textcolor{purple}{-}}, end with \tcbox{.} and can be queried by their label prefixed with \tcbox{color{purple}{-}}, e.g.\tcbox{\textcolor{purple}{-}label.}.\linebreak\linebreak
	...
	\textls[-50]{\mono{
		The most important features of bonds are \tcbox{config} and \tcbox{rules}, combining to form \tcbox{hooks}, which quantify the link between two names and give meaning to the \tcbox{\textcolor{purple}{-}} representation. Hooks are contractual mappings \tcbox{rule $\rightarrow$ config} between two names mediated by ordered one-to-one mapping inside \tcbox{\textcolor{blue}{uint8}[] rules}: \tcbox{\textcolor{darkgreen}{mapping}(\textcolor{blue}{uint8} $\rightarrow$ \textcolor{blue}{address}) hooks;}\linebreak\linebreak
		\hspace*{0.2in}$\bullet$ To query a hook's config inside \tcbox{hooks} for a bond, one needs its associated \tcbox{rule}, which is a \tcbox{uint8} identifier mapping to the contractual address \tcbox{config}. Hooks are thus queryable as \tcbox{\textcolor{purple}{-}label\textcolor{purple}{\textcolor{purple}{\#}}rule.}, e.g. \tcbox{\textcolor{purple}{-}label\textcolor{purple}{\textcolor{purple}{\#}}404.}.\linebreak\linebreak
		\hspace*{0.2in}$\bullet$ A trivial application of a hook is a payment router, i.e. payment sent to \tcbox{0} hook \tcbox{\textcolor{purple}{-}label\textcolor{purple}{\textcolor{purple}{\#}}0.} is routed to the address of \tcbox{bob.}; more on hooks in upcoming sections.\linebreak
		}}\linebreak\linebreak
...
\begin{flushleft}
	\textls[-50]{\mono{
		...
		Other features of a molecule are similar to that of a bond, e.g. a molecule can have an label and hooks. To denote a bond with label \tcbox{label}, we use two consecutive \tcbox{\textcolor{purple}{-\,-}} characters, i.e. \tcbox{\textcolor{purple}{-\,-}label.} without refering to an anion since molecules are anion-agnostic and hook-independent. By using \tcbox{\textcolor{purple}{-\,-}label\textcolor{purple}{\textcolor{purple}{\#}}rule.}, one can refer to a unique hook for a molecule. Covalence can be set (\tcbox{true}) or unset (\tcbox{false}) through co-signing a message by the cation and all the anions.
		}}\linebreak\linebreak\linebreak
	\textbf{\large\pbold{POLYCULES}}
\end{flushleft}
\lstset{emph={rules,hooks,anions}, emphstyle=\textcolor{red}}
\begin{lstlisting}
...
\begin{flushleft}
	\textls[-50]{\mono{
		Lastly, we can define another useful abstraction in the form of a 'polycule', which is a molecule comprising of unique bonds between a cation and a set of anions. In a polycule, all individual bonds share the same covalence despite being unique. In short, \tcbox{rules\textcolor{orange}{.length} == anions\textcolor{orange}{.length}}, and \tcbox{rules} \& \tcbox{anions} are a one-to-one map.\linebreak\linebreak
		Other features of a molecule are similar to that of a molecule. To denote a bond with label \tcbox{label}, we use three consecutive \tcbox{\textcolor{purple}{-\,-\,-}} characters, i.e. \tcbox{\textcolor{purple}{-\,-\,-}label.}  etc. By using \tcbox{\textcolor{purple}{-\,-\,-}label\textcolor{purple}{\#}rule.}, one can refer to a unique hook for a molecule by its \tcbox{rule}. Alternatively, one can refer to a unique anion in a molecule by its indexed \tcbox{rule}, e.g. \tcbox{\textcolor{purple}{-\,-\,-}label\textcolor{purple}{\#}anions[rule]}. Covalence can be set or unset through co-signing a message by the cation and all the anions.
	}}
\end{flushleft}
\begin{figure}[ht!]
	\centering\includegraphics[width=160mm]{../schema/helix2-struct.png}
	\mono{\caption{Helix2 Classes}}
	\label{fig:classes}
\end{figure}
\begin{flushleft}
	\textls[-50]{\mono{
		The \tcbox{molecule} structure is a topological superset i.e. it is possible to derive polycules and bonds from molecules although that'll literally be a gas-guzzling mistake. The seemingly unnecessary differentiation between the three is to optimise gas consumption.
		}}\linebreak\linebreak\linebreak
...
\end{flushleft}
\begin{figure}[ht!]
	\centering\includegraphics[width=160mm]{../schema/helix2-contracts.png}
...
\begin{flushleft}
	\textls[-50]{\mono{
		...
		The four submodules are connected at the core by the Helix2 Core manager which is the bespoke state-of-the-art Multi-facet Proxy (EIP-2535). EIP-2535 compliant contracts are fully upgradeable and it allows Helix2 protocol to use one single address forever without compromising on future functionalities or breaking existing on-chain or off-chain functionalities. Helix2 manager is capable of excepting new submodules, replace existing submodules or update core configuration.\linebreak\linebreak
		}}\linebreak\linebreak
	\textbf{\Large\pbold{UTILITY}}\linebreak\linebreak
	\textls[-50]{\mono{
		Helix2 is a core infrastructure which can be leveraged for perhaps countless utilities. Some of the notable few that come to mind are the following.\linebreak\linebreak
	}}
	\textbf{\large\pbold{MULTI-SIG VAULTS}}\linebreak\linebreak
	\textls[-50]{\mono{
		By design, Helix2 molecules are a perfect fit for multi-sig vault management. Molecule struct is readily mappable to a multi-sig safe architecture and formalise vaults as nameable linkspaces.\linebreak\linebreak
	}}
	\textbf{\large\pbold{STEALTH PAYMENTS}}\linebreak\linebreak
	\textls[-50]{\mono{
		Helix2 hooks can be configured for bonds to receive stealth payments to a name via a stealth protocol such as \href{https://github.com/ScopeLift/umbra-protocol}{Umbra Protocol} or \href{https://zk.money/}{Aztec}. Note that this feature is also possible with ENS Resolver.\linebreak\linebreak
	}}
	\textbf{\large\pbold{WEB3 SOCIAL GRAPHS}}\linebreak\linebreak
	\textls[-50]{\mono{
		Helix2 molecules are a natural fit for open social graphs and could aid in better decentralised social media protocols.\linebreak\linebreak
	}}
	...
	\textbf{\large\pbold{OTHER}}\linebreak\linebreak
	\textls[-50]{\mono{
		...
		\textbf{\Large\pbold{CLOSING REMARKS}}\linebreak\linebreak
		\textls[-50]{\mono{The architecture of a link service, Helix2, described in this paper shows several advances and features with clear utility across the Ethereum ecosystem.}}
	}}
\end{flushleft}
\begin{multicols}{2}
